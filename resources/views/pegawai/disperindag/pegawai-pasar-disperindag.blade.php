<x-pegawai-layout title="Data Pasar">
    ...
    <main class="flex-1 p-6 max-md:p-4 bg-gray-10 border-gray-20 border-[3px] rounded-[20px] mt-4">
        ...
        <div class="w-full flex items-center gap-2 mb-4 flex-wrap">
            <a href="{{ route('pegawai.disperindag.pasar.index') }}" class="text-dark flex-shrink-0">
                ...
                <table class="min-w-full text-xs md:text-sm divide-y">
                    <thead class="bg-gray-100">
                        <tr>
                            ...
                            <th rowspan="2" class="px-2 py-2 text-center">Nama Pasar</th>
                        ...
                        <tr class="hover:bg-gray-50">
                            ...
                            <td class="px-2 py-2 text-center">
                                <div class="flex justify-center gap-1">
                                    <a href="{{ route('pegawai.disperindag.pasar.edit', $item->id) }}">
...
<script>
const search=$("#search");function toggleModal(){const t=document.getElementById("filterModal");t.classList.toggle("hidden"),t.classList.toggle("flex")}search.on("input",(function(){const t=$(this).val().toLowerCase();$(".pasar_col").each((function(){$(this).text().toLowerCase().includes(t)?$(this).parent().removeClass("hidden"):$(this).parent().addClass("hidden")}))})),$("#filterBtn").on("click",(function(){$("#filterModal").toggleClass("hidden")})),$(document).on("click",".deleteBtn",(function(){let t=$(this).data("id");$("#modal").show(),$("#yesBtn").on("click",(function(){$("#modal").hide(),$.ajax({type:"DELETE",url:`/api/pasar/${t}`,data:{_token:"{{ csrf_token() }}"},success:function(t){Swal.fire({title:"Berhasil!",text:`${t.message}`,icon:"success",confirmButtonText:"OK"}).then((()=>{location.reload()}))},error:function(t,e,n){Swal.fire({icon:"error",title:"Oops...",html:n})}})}))})),$("#closeBtn").on("click",(function(){$("#modal").hide()})),$(document).ready((function(){$("#submitBtn").on("click",(function(){document.querySelector("#filterForm").submit()}))}));
</script>