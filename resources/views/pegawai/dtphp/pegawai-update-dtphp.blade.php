...
<script>
$(document).ready((function(){$("#submitBtn").on("click",(function(){$.ajax({type:"PUT",url:"{{ route('api.dtphp.update', $data->id) }}",data:{_token:"{{ csrf_token() }}",jenis_tanaman_id:$("#jenis_tanaman_id").val(),ton_volume_produksi:$("#ton_volume_produksi").val(),hektar_luas_panen:$("#hektar_luas_panen").val(),tanggal_input:$("#tanggal_input").val()},success:function(a){Swal.fire({icon:"success",title:"Berhasil",text:"Data berhasil diperbarui!",confirmButtonColor:"#16a34a"}).then((()=>{window.location.href="{{ route('pegawai.dtphp.detail.produksi') }}"}))},error:function(a,t,n){let e=a.responseJSON.errors,o="";$.each(e,(function(a,t){o+=t+"<br>"})),Swal.fire({icon:"error",title:"Oops...",html:o,confirmButtonColor:"#16a34a"})}})}))}));
</script>