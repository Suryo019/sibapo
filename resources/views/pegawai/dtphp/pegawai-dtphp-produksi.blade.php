<x-pegawai-layout title="Visualisasi Data Produksi">
    ...
    <main class="flex-1 p-6 max-md:p-4 bg-gray-10 border-gray-20 border-[3px] rounded-[20px] relative">
        <div class="w-full flex items-center justify-between gap-2 mb-5 max-md:flex-col max-md:items-start max-md:gap-1">
          ...
          <div class="max-md:my-3">
            <a href="{{ route('pegawai.dtphp.detail.produksi') }}" class="flex items-center text-lg font-semibold max-md:text-base w-full text-pink-650 gap-3">LIHAT DETAIL <i class="bi bi-arrow-right font-bold"></i></a>
          </div>
        </div>
        ...
        <div class="flex w-auto ml-4" id="switch_button">
          <a href="{{ route('pegawai.dtphp.produksi') }}">
              <button class="text-pink-500 rounded-t-xl bg-white px-4 py-3 shadow-md text-sm border bg-gray-10 border-gray-20 {{ request()->routeIs('pegawai.dtphp.detail.produksi') ? 'font-bold' : '' }} max-md:text-xs max-md:px-3 max-md:py-2 max-md:left-2">
                  Volume Produksi
              </button>
          </a>
          <a href="{{ route('pegawai.dtphp.panen') }}">
              <button class="text-gray-400 rounded-t-xl bg-gray-100 px-4 py-3 shadow-md text-sm border bg-gray-10 border-gray-20 {{ request()->routeIs('pegawai.dtphp.detail.panen') ? 'font-bold' : '' }} max-md:text-xs max-md:px-3 max-md:py-2">
                  Luas Panen
              </button>
          </a>
        </div>
    ...
    <script>
const periode=$("#pilih_periode"),search=$("#search"),container=$("#chart_container");function toggleModal(){const e=document.getElementById("filterModal");e.classList.toggle("hidden"),e.classList.toggle("flex")}$("#filterBtn").on("click",(function(){$("#filterModal").toggleClass("hidden")}));let charts=[];function renderChartFromData(e){const t=e.data;if(container.empty(),!t||0===Object.keys(t).length)return container.html('\n              <div class="text-center p-4 border-2 border-dashed border-gray-300 rounded-lg shadow-md bg-gray-50">\n                <h3 class="text-lg font-semibold text-gray-500">Data Tidak Ditemukan</h3>\n                <p class="text-gray-400">Tidak ada data untuk kriteria yang dipilih.</p>\n              </div>\n            '),void $("#switch_button").addClass("hidden");$("#switch_button").removeClass("hidden");const o=[],a={};Object.keys(t).forEach((e=>{t[e].forEach((t=>{const n=t.jenis_tanaman;o.includes(n)||o.push(n),a[e]||(a[e]={}),a[e][n]=(a[e][n]||0)+t.ton_volume_produksi}))}));const n=Object.keys(a).map((e=>({name:e,data:o.map((t=>a[e][t]||0))}))),r="chart-gabungan",i=periode.val(),[d,s]=i.split("-"),c=`Produksi Tanaman Bulan ${["Januari","Februari","Maret","April","Mei","Juni","Juli","Agustus","September","Oktober","November","Desember"][parseInt(s)-1]} ${d}`;$("#chart_container").append(`\n            <div class="mb-5 w-full rounded-2xl bg-white shadow-md p-4 border">\n              <h2 class="text-center text-lg font-semibold text-gray-700 mb-2">${c}</h2>\n              <div id="${r}" class="shadow border rounded-md p-2 bg-white"></div>\n            </div>\n          `);const l={chart:{id:`${r}_main`,type:"bar",height:450,stacked:!1,toolbar:{show:!0,tools:{download:!0,selection:!0,zoom:!0,zoomin:!0,zoomout:!0,pan:!0,reset:!0,customIcons:[{icon:'<iconify-icon icon="teenyicons:pdf-solid"></iconify-icon>',index:-1,title:"Download PDF",class:"custom-download-pdf",click:function(e,t,o){ApexCharts.exec(`${r}_main`,"dataURI").then((({imgURI:e})=>{$.ajax({url:"/export-pdf-chart",type:"POST",data:{_token:"{{ csrf_token() }}",image:e,title:c},xhrFields:{responseType:"blob"},success:function(e){const t=window.URL.createObjectURL(e),o=document.createElement("a");o.href=t,o.download=`${c.replace(/\s+/g,"_")}_${(new Date).toISOString().split("T")[0]}.pdf`,document.body.appendChild(o),o.click(),o.remove(),window.URL.revokeObjectURL(t)},error:function(e){console.error("PDF Export Error:",e.responseText),alert("Gagal mengunduh PDF. Silakan coba lagi.")}})})).catch((function(e){console.error("Chart image generation error:",e),alert("Gagal menggenerate gambar chart untuk PDF.")}))}}]}},animations:{enabled:!0,easing:"easeinout",speed:800},selection:{enabled:!0,type:"x",fill:{color:"#24292e",opacity:.1},stroke:{width:1,dashArray:3,color:"#24292e",opacity:.4}},zoom:{enabled:!0,type:"x",autoScaleYaxis:!0}},series:n,xaxis:{categories:o,title:{text:"Jenis Tanaman"}},yaxis:{title:{text:"Produksi (Ton)"},labels:{formatter:e=>e.toLocaleString("id-ID")+" Ton"}},tooltip:{y:{formatter:e=>`${e} ton`}},legend:{position:"top",onItemClick:{toggleDataSeries:!0}},responsive:[{breakpoint:768,options:{chart:{height:300},legend:{position:"bottom"}}}]};new ApexCharts(document.querySelector(`#${r}`),l).render()}function fetchDataAndRenderChart(){$.ajax({type:"GET",url:"{{ route('api.dtphp.produksi') }}",data:{_token:"{{ csrf_token() }}",periode:periode.val()},success:function(e){console.log(e),$("#periode_placeholder").html(`- ${e.periode.toUpperCase()}`),renderChartFromData(e)},error:function(e){$("#chart_container").html('\n                <div class="text-center p-4 border-2 border-dashed border-red-200 rounded-lg shadow-md bg-red-50">\n                  <h3 class="text-lg font-semibold text-red-500">Error</h3>\n                  <p class="text-red-400">Gagal memuat data. Silakan coba lagi.</p>\n                </div>\n              '),console.error("AJAX Error:",e.responseText)}})}$(document).ready((function(){fetchDataAndRenderChart()})),$("#filterSubmitBtn").on("click",(function(){fetchDataAndRenderChart()})),search.on("input",(function(){const e=$(this).val().toLowerCase();$(".nama_tanaman").each((function(){$(this).text().toLowerCase().includes(e)?$(this).parent().removeClass("hidden"):$(this).parent().addClass("hidden")}))}));
    </script>