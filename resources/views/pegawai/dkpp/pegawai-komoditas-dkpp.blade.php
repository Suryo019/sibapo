...
<x-pegawai-layout title="Data Komoditas">
    ...
    <main class="flex-1 p-6 max-md:p-4 bg-gray-10 border-gray-20 border-[3px] rounded-[20px] mt-4">
        ...
        <div class="w-full flex items-center gap-2 mb-4 flex-wrap">
            <a href="{{ route('pegawai.jenis-komoditas.index') }}" class="text-dark flex-shrink-0">
                <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="4" stroke="currentColor" class="w-6 h-6">
                    <path stroke-linecap="round" stroke-linejoin="round" d="M15.75 19.5L8.25 12l7.5-7.5" />
                </svg>                      
            </a>
            <h3 class="text-lg md:text-2xl font-semibold text-black">{{ $title }}</h3>
        </div>
                ...
                <table class="min-w-full text-xs md:text-sm divide-y">
                    ...
                    <tbody class="divide-y">
                        ...
                        <tr class="hover:bg-gray-50">
                            ...
                            <td class="px-2 py-2 text-center komoditas_col">{{ $item['nama_komoditas'] }}</td>
                            <td class="px-2 py-2 text-center">
                                <div class="flex justify-center gap-1">
                                    <a href="{{ route('pegawai.jenis-komoditas.edit', $item->id) }}">
                                    ...
                                    <button class="deleteBtn bg-red-500 text-white rounded-md w-10 h-10 md:w-10 md:h-10 flex items-center justify-center"
                                        data-id="{{ $item['id'] }}"
                                        data-nama-jenis-komoditas="{{ $item['nama_komoditas'] }}"
                                        >
...
<script>
const search=$("#search");function toggleModal(){const t=document.getElementById("filterModal");t.classList.toggle("hidden"),t.classList.toggle("flex")}search.on("input",(function(){const t=$(this).val().toLowerCase();$(".komoditas_col").each((function(){$(this).text().toLowerCase().includes(t)?$(this).parent().removeClass("hidden"):$(this).parent().addClass("hidden")}))})),$("#filterBtn").on("click",(function(){$("#filterModal").toggleClass("hidden")})),$(document).on("click",".deleteBtn",(function(){let t=$(this).data("id");$("#modal").show(),$("#yesBtn").on("click",(function(){$("#modal").hide(),$.ajax({type:"DELETE",url:`/api/jenis-komoditas/${t}`,data:{_token:"{{ csrf_token() }}"},success:function(t){Swal.fire({title:"Berhasil!",text:`Data komoditas ${t.data.nama_komoditas} telah diperbarui.`,icon:"success",confirmButtonText:"OK"}).then((()=>{location.reload()}))},error:function(t,e,o){Swal.fire({icon:"error",title:"Oops...",html:o})}})}))})),$("#closeBtn").on("click",(function(){$("#modal").hide()})),$(document).ready((function(){$("#submitBtn").on("click",(function(){document.querySelector("#filterForm").submit()}))}));
</script>