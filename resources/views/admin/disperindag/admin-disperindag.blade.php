                ...
                <form action="" method="get">
                    <div class="space-y-4">
                        ...
                        {{-- <div class="flex flex-col">
                            ...
                            <select name="bahan_pokok" id="pilih_bahan_pokok" class="border w-full max-md:w-full p-2 rounded bg-white select2 text-xs">
                                <option value="" disabled {{ old('bahan_pokok') ? '' : 'selected' }}>Pilih Bahan Pokok</option>
                                @foreach ($data as $index => $item)
                                    <option value="{{ $item->nama_bahan_pokok }}"
                                        {{ old('bahan_pokok') == $item->nama_bahan_pokok ? 'selected' : ($index == 0 && !old('bahan_pokok') ? 'selected' : '') }}>
                                        {{ $item->nama_bahan_pokok }}
                                    </option>
                                @endforeach
                            </select>
                        </div> --}}
    ...
    <main class="flex-1 p-6 max-md:p-4 bg-gray-10 border-gray-20 border-[3px] rounded-[20px]">
        <div class="w-full flex items-center justify-between gap-2 mb-5 max-md:flex-col max-md:items-start max-md:gap-1">
            ...
            <div class="max-md:my-3">
                <a href="{{ route('disperindag.detail') }}" class="flex items-center text-lg font-semibold max-md:text-base w-full text-pink-650 gap-3">LIHAT DETAIL <i class="bi bi-arrow-right font-bold"></i></a>
            </div>
        </div>
...
<script>
    ...
    // let pasar = 'Pasar Tanjung';
    // let periode = 'May 2025';
    // let bahanPokok = 'Minyak Goreng';
