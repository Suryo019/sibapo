            ...
            <x-filter-modal>
                <form action="" method="get" class="space-y-4">
                    ...
                    <div class="flex flex-col">
                        ...
                        <select name="periode" class="border border-black p-2 rounded-lg bg-white w-full select2" id="pilih_periode">
...
<script>
function toggleModal(){const t=document.getElementById("filterModal");t.classList.toggle("hidden"),t.classList.toggle("flex")}$("#filterBtn").on("click",(function(){$("#filterModal").toggleClass("hidden")})),$("#closeListModal").on("click",(function(){$(this).closest("#modal").removeClass("flex").addClass("hidden")})),$(document).on("click",".btnConfirm",(function(){let t=$(this).data("id");$("#deleteModal").show(),$("#yesBtn").off("click").on("click",(function(){$.ajax({type:"DELETE",url:`/api/dpp/${t}`,success:function(t){Swal.fire({title:"Berhasil!",text:`Data ${t.data.jenis_bahan_pokok} telah dihapus.`,icon:"success",confirmButtonText:"OK"}).then((()=>{location.reload()}))},error:function(t,e,o){Swal.fire({icon:"error",title:"Oops...",html:o})}}),$("#deleteModal").hide()}))})),$(document).on("click","#closeBtn",(function(){$("#deleteModal").hide()})),$(document).ready((function(){$("#submitBtn").on("click",(function(){document.querySelector("#filterForm").submit()}))}));
</script>