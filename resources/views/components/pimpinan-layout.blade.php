...
<body class="h-screen w-screen overflow-hidden">
    ...
    <script>
        $(document).ready((function(){$(".select2").select2()}));const API_TOKEN="{{ session('api_token') }}";function showLoading(){document.getElementById("loading").style.display="flex"}function hideLoading(){document.getElementById("loading").style.display="none"}$.ajaxSetup({headers:{"X-CSRF-TOKEN":$('meta[name="csrf-token"]').attr("content"),Authorization:"Bearer "+API_TOKEN}}),$(document).ajaxStart((function(){$("#loading").show()})).ajaxStop((function(){$("#loading").hide()}));
    </script>
