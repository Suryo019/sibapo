...
class NotifikasiDkppController extends Controller
{
    ...
    {
        $query = Notifikasi::with('role')->where('role_id', 3)->where('is_admin', 0);
    ...
    public function getHeaderNotifications()
    {
        ...
        $unreadCount = Notifikasi::where('role_id', 3)
            ->where('is_admin', 0)
            ->where('is_read', false)
            ->count();
    ...
    public function markAsRead(Request $request)
    {
        ...
        if ($notificationId) {
            ...
            Notifikasi::where('role_id', 3)->where('is_admin', 0)->update(['is_read' => true]);
        }
