...
class NotifikasiDisperindagController extends Controller
{
    ...
    {
        $query = Notifikasi::with('role')->where('role_id', 2)->where('is_admin', 0);
        ...
        if ($request->ajax()) {
            return response()->json([
                'success' => true,
                'html' => view('pegawai.partials.notifikasi-list', compact('notifikasis'))->render()
            ]);
        }
    ...
    public function getHeaderNotifications()
    {
        ...
        $unreadCount = Notifikasi::where('role_id', 2)
            ->where('is_admin', 0)
            ->where('is_read', false)
            ->count();
    ...
    public function markAsRead(Request $request)
    {
        ...
        if ($notificationId) {
            ...
            Notifikasi::where('role_id', 2)->where('is_admin', 0)->update(['is_read' => true]);
        }
